...
export default async function DealsPage({ params }: { params: { orgId: string } }) {
    const data = await getData(params.orgId)

    return (
        <div className="container mx-auto py-10 h-full">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">Deals</h1>
                <Link href={`/${params.orgId}/deals/new`}>
                    <Button>
                        <Plus className="mr-2 h-4 w-4" /> New Deal
                    </Button>
                </Link>
            </div>
            <DealsView deals={data} orgId={params.orgId} />
        </div>
    )
}
